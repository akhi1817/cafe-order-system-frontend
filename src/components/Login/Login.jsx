...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(API_ENDPOINTS.LOGIN_USER, { email, password }, { withCredentials: true });
      const user = res.data.user;
      if (!user) { toast.error("User data not found"); return; }
    ...
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-green-100 via-green-200 to-yellow-100 p-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="backdrop-blur-xl bg-white/60 shadow-xl border border-white/20 p-8 rounded-3xl w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-5 flex flex-col items-center">
          {loading ? <Shimmer /> : <input type="email" placeholder="Enter your email" value={email} onChange={e => setEmail(e.target.value)} required className="w-full h-12 border border-green-300 bg-white/70 rounded-xl px-4 focus:ring-2 focus:ring-green-400 outline-none" />}
          {loading ? <Shimmer /> : <input type="password" placeholder="Enter your password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full h-12 border border-green-300 bg-white/70 rounded-xl px-4 focus:ring-2 focus:ring-green-400 outline-none" />}
